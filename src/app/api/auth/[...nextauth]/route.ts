...
import { users, userSystemRoles, rolePermissions, permissions as permissionsTable } from "@/db/schema";
...
export const authOptions: NextAuthOptions = {
    providers: [
        CredentialsProvider({
            name: "Credentials",
            credentials: {
                dni: { label: "DNI o Usuario", type: "text" },
            ...
            async authorize(credentials) {
                ...
                try {
                    ...
                    // Validate BackEpi module access
                    const assignedRoles = await db.select()
                        .from(userSystemRoles)
                        .where(eq(userSystemRoles.userId, user.id));

                    // Check if they have access to backepi
                    const backepiRole = assignedRoles.find((r: any) => r.systemId === "backepi" || r.systemId === "admin");
                    if (!backepiRole) {
                    ...
                    const permissionRows = await db.select({
                        ...
                        .where(eq(rolePermissions.roleId, backepiRole.roleId));
    ...
    callbacks: {
        async jwt({ token, user }) {
            if (user) {
                ...
                token.backepiRoleId = (user as any).roleId;
        ...
        async session({ session, token }) {
            if (token && session.user) {
                ...
                (session.user as any).backepiRoleId = token.backepiRoleId;
