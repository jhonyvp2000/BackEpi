...
import { saveMonthlyIndicator } from '@/app/actions/indicadoresMensuales';

interface IndicadoresModalProps {
    isOpen: boolean;
    onClose: () => void;
    activeTab: string; // 'rendimiento' | 'analisis'
}

export function IndicadoresModal({ isOpen, onClose, activeTab }: IndicadoresModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await saveMonthlyIndicator(formData);

            if (result.success) {
                alert('Boletín mensual guardado correctamente.');
                setFileName(null);
                formElement.reset();
                onClose();
            } else {
    ...
    switch (activeTab) {
        case 'rendimiento':
            title = "Subir Rendimiento Hora Médico";
            buttonColor = "bg-[var(--color-hospital-blue)] hover:bg-blue-700";
            break;
        case 'analisis':
            title = "Subir Análisis de Indicadores Hospitalarios";
                    ...
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div className="space-y-2">
                                ...
                                <select name="mes" required className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-slate-900 dark:text-white dark:bg-slate-800 dark:border-slate-700 focus:border-[var(--color-hospital-blue)] focus:outline-none focus:ring-1 focus:ring-[var(--color-hospital-blue)]">
                        ...
                        <div className="mb-6">
                            ...
                            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 dark:hover:bg-bray-800 dark:bg-slate-800 hover:bg-slate-100 dark:border-slate-600 dark:hover:border-slate-500 dark:hover:bg-slate-700 transition">
                                ...
                                <input name="boletinFile" id="dropzone-file" type="file" className="hidden" accept=".pdf" onChange={handleFileChange} required />
                            </label>
                        </div>
